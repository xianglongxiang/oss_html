...
var config = {
    ...
    right: -232,
    api: {
        friend: 'js/plugins/layer/layim/page/friend.json', //好友列表接口
        group: 'js/plugins/layer/layim/page/group.json', //群组列表接口
        chatlog: 'js/plugins/layer/layim/page/chatlog.json', //聊天记录接口
        groups: 'js/plugins/layer/layim/page/groups.json', //群组成员接口
        sendurl: '' //发送消息接口
    },
    user: { //当前用户信息
        name: '游客',
...
xxim.popchat = function(param){
    ...
    log.success = function(layero){
        ...
        log.chatlist.html('<li page-id="'+ param.id +'" type="'+ param.type +'"  id="layim_user'+ param.type + param.id +'"><span>'+ param.name +'</span><em>×</em></li>')
        ...
        log.chatlist.on('click', 'li em', function(e){
            ...
            var dataId = parents.attr('page-id'), index = parents.index();
            ...
            if(parents.hasClass('layim_chatnow')){
                ...
                xxim.tabchat(config.chating[chatlist.eq(indexs).attr('type') + chatlist.eq(indexs).attr('page-id')]);
            }
        ...
        log.chatlist.on('click', 'li', function(){
            var othis = $(this), dataType = othis.attr('type'), dataId = othis.attr('page-id');
    ...
    if(config.chatings < 1){
        ...
        log.chatmore.find('ul').append('<li page-id="'+ param.id +'" type="'+ param.type +'" id="layim_user'+ param.type + param.id +'" class="layim_chatnow"><span>'+ param.name +'</span><em>×</em></li>');
...
xxim.popchatbox = function(othis){
    var node = xxim.node, dataId = othis.attr('page-id'), param = {
        id: dataId, //用户ID
...
xxim.getGroups = function(param){
    ...
    config.json(config.api.groups, {}, function(datas){
        if(datas.status === 1){
            ...
            if(lens > 0){
                for(; ii < lens; ii++){
                    str += '<li page-id="'+ datas.data[ii].id +'" type="one"><img src="'+ datas.data[ii].face +'" class="xxim_oneface"><span class="xxim_onename">'+ datas.data[ii].name +'</span></li>';
                }
            } else {
...
xxim.transmit = function(){
    ...
    log.send = function(){
        ...
        if(data.content.replace(/\s/g, '') === ''){
            ...
            /*
            that.json(config.api.sendurl, page, function(datas){
    ...
    config.json(api[index], {}, function(datas){
        if(datas.status === 1){
            ...
            if(myflen > 1){
                if(index !== 2){
                    for(; i < myflen; i++){
                        str += '<li page-id="'+ datas.data[i].id +'" class="xxim_parentnode">'
                        ...
                        for(var j = 0; j < item.length; j++){
                            str += '<li page-id="'+ item[j].id +'" class="xxim_childnode" type="'+ (index === 0 ? 'one' : 'group') +'"><img src="'+ item[j].face +'" class="xxim_oneface"><span class="xxim_onename">'+ item[j].name +'</span></li>';
                        }
                        str += '</ul></li>';
                    }
                } else {
                    str += '<li class="xxim_liston">'
                        +'<ul class="xxim_chatlist">';
                    for(; i < myflen; i++){
                        str += '<li page-id="'+ datas.data[i].id +'" class="xxim_childnode" type="one"><img src="'+ datas.data[i].face +'"  class="xxim_oneface"><span  class="xxim_onename">'+ datas.data[i].name +'</span><em class="xxim_time">'+ datas.data[i].time +'</em></li>';
                    }
                    str += '</ul></li>';
                }
                myf.html(str);
